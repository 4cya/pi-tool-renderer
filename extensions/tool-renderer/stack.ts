...
function stackItemPreview(item: StackItem, theme: any, expanded: boolean): string {
	if (!item.resultText || item.status === "running") return "";
	if (item.toolName === "find" || item.toolName === "ls") return renderPathListPreview(item.resultText, item.toolName, theme, expanded);
	if (item.toolName === "bash") {
		const renderDiffs = shouldRenderBashDiffsForCommand(item.args);
		if (!renderDiffs && suppressReadOnlyBashDiffOutput(item.args, item.resultText)) return "";
		return renderBashDiffOutput(item.resultText, theme, expanded, renderDiffs) ?? preview(item.resultText, Math.max(1, Math.floor(settingNumber("bashPreviewLines", 80))), "tail");
	}
	if (item.toolName === "read") return preview(item.resultText, Math.max(1, Math.floor(settingNumber("readPreviewLines", 80))), "head");
	return preview(item.resultText, Math.max(1, Math.floor(settingNumber("searchPreviewLines", 80))), "head");
}

export function renderStackItemText(item: StackItem, theme: any, expanded: boolean, branch = "├"): string {
	...
	if (expanded) {
		const previewText = stackItemPreview(item, theme, expanded);
		if (previewText) {
			const stem = item.toolName === "bash" ? treeConnector(theme, "│") : treeStem(theme, typedBranch);
...
function renderStackBatch(batch: StackBatch, theme: any, expanded: boolean, childDisplay: StackChildDisplay = "rows"): TruncatedLines {
	...
	if (childDisplay === "anchor-list" || (childDisplay === "headline" && expanded)) {
		...
		items.forEach((item, index) => {
			text += `\n${renderStackItemText(item, theme, expanded, index === items.length - 1 ? "└" : "├")}`;
		});
	}
	return makeTruncatedLines(text);
}

export function renderStackedToolResult(toolName: StackableToolName, result: any, isPartial: boolean, expanded: boolean, theme: any, context: any) {
	...
	if (batch.anchorId === id) return renderStackBatch(batch, theme, expanded, childDisplay);
	...
	return makeTruncatedLines(renderStackItemText(item, theme, false, index === items.length - 1 ? "└" : "├"));
}
